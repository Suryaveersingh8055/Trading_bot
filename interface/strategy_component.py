...
class StrategyEditor(tk.Frame):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index] = tk.OptionMenu(self._body_frame.sub_frame,
                                                                      ...
                                                                      *base_param['values'])
                self.body_widgets[code_name][b_index].config(width=base_param['width'], highlightthickness=False, bd=0, indicatoron=0)

            elif base_param['widget'] == tk.Entry:
                self.body_widgets[code_name][b_index] = tk.Entry(self._body_frame.sub_frame, justify=tk.CENTER, highlightthickness=False,
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text") == "OFF":
            ...
            self.body_widgets['activation'][b_index].config(bg="darkred", text="OFF", width=40)
